...
export default function ProductCard({ product, priority = false }: ProductCardProps) {
  ...
  return (
    <motion.div
      ...
      className="product-card bg-white group shadow-glass hover:shadow-card-hover transition-shadow duration-500 rounded-xl overflow-hidden relative"
    >
      {/* Image Container */}
      ...
      <div className="p-4">
        ...
        {/* Price + CTAs */}
        <div className="mt-3 pt-3 border-t border-brand-50 space-y-2">
          ...
          <motion.button
            ...
            className={`w-full flex items-center justify-center gap-2 py-2.5 font-jost text-xs tracking-widest uppercase transition-all duration-300 ${
              addedSize
                ? 'bg-green-500 text-white'
                : 'bg-brand-900 text-cream hover:bg-brand-700'
            }`}
            whileTap={{ scale: 0.97 }}
          >
            <ShoppingBag size={12} />
            {addedSize ? `Added (${addedSize}) ✓` : 'Add to Cart'}
          </motion.button>
        </div>
      </div>

      {/* Size Selection Modal (inline overlay) */}
      <AnimatePresence>
        {showSizeModal && (
          <motion.div
            ...
            className="absolute inset-0 z-30 bg-cream/97 backdrop-blur-sm flex flex-col justify-center items-center p-5 rounded-xl"
          >
            <button
              onClick={() => setShowSizeModal(false)}
              className="absolute top-3 right-3 w-7 h-7 rounded-full bg-brand-100 flex items-center justify-center text-brand-500 hover:bg-brand-200 transition-colors"
            >
              <X size={13} />
            </button>
            <p className="font-cormorant text-xl text-brand-900 font-medium mb-1">
              {product.name}
            </p>
            <p className="font-jost text-[10px] text-brand-400 tracking-widest uppercase mb-4">
              Select Size to Add
            </p>
            <div className="flex flex-wrap justify-center gap-2">
              {product.sizes.map((size) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.93 }}
                  className="w-11 h-11 font-jost text-xs border border-brand-200 text-brand-700 hover:bg-brand-800 hover:text-cream hover:border-brand-800 transition-all duration-200"
                >
                  {size}
                </motion.button>
              ))}
